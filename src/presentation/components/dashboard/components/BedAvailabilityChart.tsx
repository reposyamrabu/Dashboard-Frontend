...
export const BedAvailabilityChart: React.FC<BedAvailabilityChartProps> = ({
    ...
    const chartHeight = useMemo(() => {
        if (!data || data.length === 0) return 'h-64 sm:h-80 md:h-96';
        // Dynamic height based on number of items (30px per item minimum)
        const minHeight = Math.max(data.length * 30, 400);
